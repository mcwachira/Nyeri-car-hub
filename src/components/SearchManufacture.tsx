...
const SearchManufacture = ({manufacturer, setManufacturer}:SearchManufacturerProps) => {
    ...
    const filteredManufacturer =
        ...
        item.toLowerCase()
            .replace(/\s+/g, "") //replace empty spaces with string
                    ...
                    <Transition as={Fragment} leave='transition ease-in duration-100'
                        ...
                        <Combobox.Options>
                            {filteredManufacturer.length === 0  && query !== ""? (
                                ...
                                <Combobox.Option
                                        key={item} className={({active}) => `relative search-manufacture__option ${active? 'bg-primary-blue text-white': 'text-gray-900'}`}
